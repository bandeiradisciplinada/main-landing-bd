...
const ProductPage: React.FC<Props> = ({ product }) => {
  ...
  return (
    <div className="bg-background dark:bg-gray-900 min-h-screen grid gap-y-16 overflow-hidden">
      <Head>
      ...
      <div className="relative bg-background dark:bg-gray-900">
        <div className="max-w-7xl mx-auto">
          <div className="relative z-10 pb-8 bg-background dark:bg-gray-900 sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32">
            <Header />
          </div>
        </div>
      </div>
      ...
      <section className="container mx-auto px-6 pt-10 pb-16 text-center relative z-10">
        ...
        <div className="flex justify-center">
          <img
            src={product.img}
            alt={product.title}
            className={`w-full max-w-md object-contain h-64${product.imgDark ? ' dark:hidden' : ''}`}
          />
          {product.imgDark && (
            <img
              src={product.imgDark}
              alt={product.title}
              className="w-full max-w-md object-contain h-64 hidden dark:block"
            />
          )}
        </div>
      </section>
      ...
      {product.features && product.features.length > 0 && (
        <section className="bg-tertiary dark:bg-gray-800 py-16 relative z-10">
          <div className="container mx-auto px-6 max-w-5xl">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {product.features.map((feature, idx) => (
                <div
                  key={idx}
                  className="bg-background dark:bg-gray-900 p-8 rounded-xl shadow-md border-t-4 border-primary hover:shadow-lg transition-shadow"
                >
